...
const User = {
    ...
    create: (name, email, score, password) => {
        ...
        const generateTokenAndCreate = () => {
            ...
            (name, email, score, password_digest, token)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING name, email, score, token, id`, // the information we want to send back
                        [name, email, score, password_digest, token])
                })
                .catch(err => {
                    console.log(err)
    ...
    update: (score, id) => db.one(`UPDATE users SET score=$1 WHERE id=$2 RETURNING *`, [score, id])
